...
def verificar_webhook(
    ...
    """
    Endpoint de verificación de Webhook requerido por Meta WhatsApp Cloud API.
    """
...
async def recibir_mensaje_whatsapp(request: Request):
    """
    Recibe la notificación del mensaje entrante de Meta WhatsApp Cloud API.
    """
    data = await request.json()
    logger.info(f"[WhatsApp Incoming Payload]: {data}")

    try:
        # Extraer detalles del mensaje si es una estructura válida de Meta Cloud API
...
async def enviar_mensaje_whatsapp(to_phone: str, text: str):
    """
    Envía un mensaje de texto de vuelta al usuario usando la API Oficial de Meta.
    """
...
def simular_chat(payload: WhatsAppMessagePayload):
    """
    Endpoint para probar directamente desde la consola o Postman/Swagger sin necesidad de Meta.
    """
